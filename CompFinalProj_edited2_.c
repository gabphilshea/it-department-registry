...
typedef struct Student {
    ...
    int id;                  // student ID
    char contact_number[11];      // contact number (FIXED: no space!)
    float gwa;               // general weighted average (FIXED: gwa not gpa)
    int is_inc;              // 1 = Incomplete status, 0 = has a GWA
    char email[100];         // email address (FIXED: needs array size!)
} Record;                    // FIXED: closing brace before Record
...
void loadfromFile() {
    ...
    while (student_count < 100 &&
           fscanf(fp, "%d|%49[^|]|%f|%d\n",
                  ...
                  &student_list[student_count].is_inc) == 4) {
        student_count++;
    }
    fclose(fp);
